...
function Login({ setUser, setAlert, productCount }) {
  function callLoginApi(values) {
    axios
      .post("https://easykartbackend.onrender.com/login", {
        email: values.email,
        password: values.password,
      })
      .then((response) => {
        const data = response.data; // ✅ actual API response yahan milta hai
        localStorage.setItem("token", data.auth);
        localStorage.setItem("user", data.users.email);

        setUser(data.users.name); // ✅ user context me set karo
      })
      .catch((error) => {
      ...
      <div className="bg-white pt-8">
        ...
        appearance-none rounded-sm
         ...
         focus:z-10 focus:border-indigo-500 focus:outline-none focus:ring-indigo-500 sm:text-sm"
            ...
            <Link className="self-center mt-8" to={"/signup/"}>
              <button
                type="button"
